...
import { Search, Filter, Calendar, Users, BarChart3, TrendingUp, Download, FileText, Globe, MapPin, Building2, GraduationCap, Network } from 'lucide-react';
...
interface PageProps {
    internationalPartners: PaginationData;
    campuses: Campus[];
    colleges: College[];
    filters: {
        campus_id?: string;
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Reports',
        href: '/admin/report/international-partners',
    },
    {
        title: 'International Partners Report',
        href: '/admin/report/international-partners',
    },
];

export default function InternationalPartnersReport({ internationalPartners, campuses, colleges, filters }: PageProps) {
    ...
    const applyFilters = () => {
        ...
        router.get('/admin/report/international-partners', cleanFilters, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const clearFilters = () => {
        ...
        router.get('/admin/report/international-partners');
    };
    ...
    const generatePDF = () => {
        ...
        const url = `/admin/report/international-partners/pdf?${params.toString()}`;
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="International Partners Report" />

            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl px-10 py-5 overflow-x-auto">
                {/* Header */}
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-medium">International Partners Report</h1>
                        <p className="text-muted-foreground">
                            Comprehensive overview of all international partnership activities
                        </p>
                    </div>
                </div>
                    ...
                    <CardHeader>
                        <CardTitle className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                International Partners ({internationalPartners.total})
                            </div>
                    ...
                    <CardContent>
                        <div className="rounded-md border">
                            <Table>
                                ...
                                <TableBody>
                                    {internationalPartners.data.map((partner) => (
                                        <TableRow key={partner.id}>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <div>
                                                        <div className="flex items-center gap-1">
                                                            ...
                                                            <span className="text-xs">{partner.campus_college?.campus?.name || 'N/A'}</span>
                                                        </div>
                                                        <div className="flex items-center gap-1">
                                                            <GraduationCap className="h-3 w-3" />
                                                            <span className="text-xs">{partner.campus_college?.college?.name || 'N/A'}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </TableCell>
                                            ...
                                            <TableCell>
                                                {getStatusBadge(partner.start_date, partner.end_date)}
                                            </TableCell>
                                            <TableCell>
                                                <div className="text-sm">
                        ...
                        {/* Pagination */}
                        {internationalPartners.last_page > 1 && (
                            <div className="flex items-center justify-between px-2 py-4">
                                <div className="text-sm text-muted-foreground">
                                    Showing {internationalPartners.from} to {internationalPartners.to} of {internationalPartners.total} results
                                </div>
                                <div className="flex items-center space-x-2">
                                    {internationalPartners.prev_page_url && (
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            onClick={() => router.get(internationalPartners.prev_page_url)}
                                        >
                                            Previous
                                        </Button>
                                    )}
                                    {internationalPartners.next_page_url && (
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            onClick={() => router.get(internationalPartners.next_page_url)}
                                        >
                                            Next
                                        </Button>
                                    )}
