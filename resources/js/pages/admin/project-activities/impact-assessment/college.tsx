...
type College = BaseCollege & {
    impact_assessments_count: number;
};
                ...
                <div className="grid auto-rows-min gap-4 md:grid-cols-3 lg:grid-cols-4">
                    {colleges.map((college) => (
                        <Card key={college.id} className='hover:shadow-lg transition-shadow duration-200'>
                            <Link href={`/admin/impact-assessment/${campus.id}/${college.id}/assessments`} className="flex flex-col items-center gap-3">
                                <CardContent>
                                ...
                                >
                                    <span className="flex items-center gap-1">
                                        ...
                                        {college.impact_assessments_count} Assessment{college.impact_assessments_count > 1 ? "s" : ""}
                                    </span>
                                </Badge>
                            </Link>
                        </Card>
